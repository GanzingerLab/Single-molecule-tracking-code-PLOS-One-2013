...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 
function [results] = msd_jd_analysis(parameters,param_guess1,param_guess2,param_guess3,results,setup)


directory = strcat(parameters.exp_name,'\','spot detection Results');
[list_dir,folders] = get_folders_folders('all',directory);

dt = parameters.time/1000;
...
for ndir = 1:size(list_dir,2), %loop over all folders which contain keyword
    ...
    for ntrack = 1:Ltracks,      
        ...
        [msd_temp,se_temp,grad_temp,err_grad_temp,offset_temp] = get_msd(t,x,y,parameters);
       
        jump = get_jump(t,x,y,parameters);
...
[save_name_MSD,save_name_JD]= make_directoryMSD(parameters);
...
plot_hist_jump1(jump_all,1,dt,param_guess1);
saveas(gcf,strcat(save_name_JD,'Histogram_fit1')); 
[param] = cumulative_fit_1( jump_all,dt,param_guess1 );
...
plot_hist_jump2(jump_all,1,dt,param_guess2);
saveas(gcf,strcat(save_name_JD,'Histogram_fit2')); 
[param] = cumulative_fit_2(jump_all,dt,param_guess2);
...
[param] = cumulative_fit_3( jump_all,dt,param_guess3 );
